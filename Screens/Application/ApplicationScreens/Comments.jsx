...
const Comments = ({applicationId}) => {
    ...
    useEffect(()=>{
      ...
      setComments(responce.data.data.comments);
    ...
    const AddingComment = async()=>{
        ...
        try {
            if (newComment) {
                const responce = await axios.post(`https://dev.shabujglobal.org/api/application/${applicationId}/comment`,
                    {
        comment: newComment,
      }
    ,{ 
      ...
      <List.Accordion
          ...
          title={
            <View style={{backgroundColor:"#EEEEFF"}}>
              <Text style={{color:"#0052FF", fontFamily:"Montserrat_700Bold",}}>Comment</Text>
            </View>
          }
          >
        <View style={{padding:10, backgroundColor:"#E8E8F1"}}>
            ...
            renderItem={({item})=>(
                <View style={{flexDirection:"row", alignItems:"center", justifyContent:"space-between", gap:5}}>
                    ...
                    <Text style={{fontSize:16, fontFamily:"Montserrat_400Regular"}}>{item.comment.replace(/<[^>]*>/g, '')}</Text>
                </View>
                </View>
            )}
            />
